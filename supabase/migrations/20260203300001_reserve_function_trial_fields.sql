-- Update reserve_discount_code to return trial period fields
--
-- Adds plan_trial_price and plan_trial_duration_months to the return type
-- so the checkout UI can display trial pricing when a discount code maps
-- to a plan with a trial period.
...
CREATE FUNCTION reserve_discount_code(
  ...
  plan_trial_price DECIMAL,
  plan_trial_duration_months INT,
  error_code TEXT,
  error_message TEXT
) AS $$
DECLARE
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT
      FALSE, NULL::UUID, NULL::UUID, NULL::TEXT, NULL::DECIMAL, NULL::TEXT,
      NULL::TEXT, NULL::TEXT, NULL::DECIMAL, NULL::DECIMAL, NULL::INT,
  ...
  IF FOUND THEN
    ...
    RETURN QUERY SELECT
      ...
      v_plan.trial_price_amount,
      v_plan.trial_duration_months,
      NULL::TEXT,
      NULL::TEXT;
    RETURN;
  END IF;
  ...
  -- Return success with plan details including trial info
  RETURN QUERY SELECT
    TRUE,
    v_reservation_id,
    v_plan.id,
...
COMMENT ON FUNCTION reserve_discount_code IS
  'Atomically validates and reserves a discount code. IDEMPOTENT: Returns existing valid reservation if one exists for this email+code. Returns plan details including trial period info for checkout display. Reservation expires in 15 minutes.';
