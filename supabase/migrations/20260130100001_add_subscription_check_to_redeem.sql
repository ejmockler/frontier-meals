-- Add subscription status check to redeem_meal function (Service Gate Pattern)
-- Prevents suspended/inactive customers from redeeming meals
-- Critical security fix: MT-2
...
BEGIN
  ...
  -- Check subscription status (service gate)
...
COMMENT ON FUNCTION redeem_meal IS 'Atomically redeem a meal using QR code. Prevents race conditions by locking entitlement and QR token rows. Enforces service gate pattern by checking subscription status (active/trialing only).';
