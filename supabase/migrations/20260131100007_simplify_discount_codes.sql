-- Simplify discount codes: remove unnecessary fields
--
-- Model: Each PayPal Plan has pricing baked in. A discount code just maps CODE → Plan ID.
-- Discount is calculated at runtime as: default_plan.price - selected_plan.price
...
DROP FUNCTION IF EXISTS get_discount_display(TEXT, DECIMAL, INT);
...
ALTER TABLE discount_codes
  DROP CONSTRAINT IF EXISTS chk_discount_value_positive,
  DROP CONSTRAINT IF EXISTS chk_discount_duration_positive,
  DROP CONSTRAINT IF EXISTS chk_percentage_max_100;
...
-- ============================================================================
-- STEP 4: Drop and recreate reserve_discount_code() function
-- Now returns default plan price alongside discount plan price for delta calculation
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT
      FALSE, NULL::UUID, NULL::UUID, NULL::TEXT, NULL::DECIMAL, NULL::TEXT, NULL::TEXT, NULL::DECIMAL,
  ...
  -- Return success with both plan price and default price for delta calculation
  RETURN QUERY SELECT
    TRUE,
    v_reservation_id,
    v_plan.id,
...
COMMENT ON FUNCTION reserve_discount_code IS
  'Atomically validates and reserves a discount code. Returns plan details plus default plan price for discount calculation. Reservation expires in 15 minutes.';
...
COMMENT ON TABLE discount_codes IS
  'Promotional codes that map to specific PayPal subscription plans. Discount is calculated as delta from default plan price.';
